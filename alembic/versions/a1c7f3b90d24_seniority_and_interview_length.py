...
def upgrade() -> None:
    ...
    op.add_column(
        "conversations",
        sa.Column("seniority_source", sa.Text(), nullable=False, server_default="fallback"),
    )
    op.add_column("conversations", sa.Column("seniority_evidence", sa.Text(), nullable=True))
    op.add_column(
        "conversations",
        sa.Column("interview_length", sa.Text(), nullable=False, server_default="standard"),
    )
    # Nullable on purpose: legacy rows fall back to INTERVIEW_MAX_MINUTES.
    op.add_column("conversations", sa.Column("max_minutes", sa.Integer(), nullable=True))
    op.add_column("milestones", sa.Column("expected_evidence", sa.Text(), nullable=True))
    op.add_column("evaluations", sa.Column("seniority_evaluated", sa.Text(), nullable=True))
    op.add_column(
        "evaluations",
